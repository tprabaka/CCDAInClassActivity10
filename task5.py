...
if not os.path.exists(MODEL_PATH):
    ...
    hist_windowed_df = hist_df_processed.groupBy(window(col('event_time'),'5 minutes')).agg(avg('fare_amount').alias('avg_fare'))
    ...
    hist_features = hist_features.withColumn('minute_of_hour', minute(col("window.start")))
...
windowed_df = parsed_stream.groupBy(window(col('event_time'),'5 minutes','1 minutes')).agg(avg(col('fare_amount')).alias('avg_fare'))
...
windowed_features=windowed_features.withColumn('minute_of_hour', minute(col("window.start")))
